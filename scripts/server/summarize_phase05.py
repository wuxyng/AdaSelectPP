...
def _summarize_pair_supply_metrics(rows: List[Dict[str, str]]) -> List[str]:
    ...
    lines = [
        f"- candidate_generation_modes: {_top_counter(Counter(str(r.get('candidate_generation_mode', '') or 'unknown') for r in rows))}",
        f"- cg_width2_pre_cap_total: {_fmt(sum(_as_float(r.get('cg_width2_pre_cap_count')) for r in rows))}",
        f"- cg_width2_post_cap_total: {_fmt(sum(_as_float(r.get('cg_width2_post_cap_count')) for r in rows))}",
        f"- cg_width2_dropped_round_total: {_fmt(sum(_as_float(r.get('cg_width2_dropped_round_count')) for r in rows))}",
        f"- cg_width2_fairness_added_total: {_fmt(sum(_as_float(r.get('cg_width2_fairness_added_count')) for r in rows))}",
        f"- cg_width2_fairness_added_pairs: {_top_counter(_example_counter(rows, 'cg_width2_fairness_added_pairs'))}",
        f"- cg_target_pair_postround_coverage_total: {_fmt(sum(_as_float(r.get('cg_target_pair_postround_coverage_count')) for r in rows))}",
        f"- cg_candidate_budget_delta_total: {_fmt(sum(_as_float(r.get('cg_candidate_budget_delta')) for r in rows))}",
